...
class CrmAiController extends Controller
{
    public function recommendNextAction(Request $request, int $leadId): JsonResponse
    {
        ...
        $estimatorUrl = config('crm.estimator_url', 'https://www.digitalbuilders.in/estimator');
        ...
        if ($lead->stage === 'new' || $touchCount === 0) {
            ...
            $recommendation['reasoning'] = "Proposal sent {$deal?->formatted_amount}. Call {$lead->name} to walk through timeline & kickoff deposit.";
            ...
            $recommendation['reasoning'] = "Anchor value transparently. Share {$estimatorUrl} so client ballparks budget.";
            ...
            $recommendation['reasoning'] = "Build deep technical trust. Send relevant case study.";
    ...
    public function generateScript(Request $request): JsonResponse
    {
        ...
        if (!str_starts_with($rawPhone, '+') && strlen($phone) === 10 && ($lead->region === 'IN' || !$lead->region)) {
            $phone = '91' . $phone;
        }
        ...
        if ($type === 'objection') {
            if (str_contains(strtolower($objection), 'price') || str_contains(strtolower($objection), 'expensive') || str_contains(strtolower($objection), '25000')) {
                ...
                $script = "Hi {$name}, I completely understand budget is important.\n\nThe reality of discount software is that it's usually built on fragile, bloated plugins with zero automated tests. Six months down the line when it crashes during peak traffic, you end up spending double to rebuild from scratch.\n\nWith us, you get clean, proprietary architecture personally reviewed by a {$founderTitle}, complete IP ownership, and a 30-day warranty. If you want to de-risk, we can start with our Architecture Discovery Sprint, which is 100% credited to the build contract once we kick off.\n\nWould you like me to reserve a 15-min discovery slot this Thursday?";
            ...
            if ($touch === 1) {
                ...
                if ($segment === 'manufacturer') {
                    $script = "Hi {$name}, saw you run {$company}. Are you still managing dealer dispatches on WhatsApp chats and handwritten slips? We built a 1-tap ordering app for Garg Enterprises in Ludhiana that dropped their dispatch mistakes from 14% down to zero. Would you be open to a 15-min look at how it works? ({$websiteUrl}/portfolio/garg-enterprises)";
                } elseif ($segment === 'retail') {
                    $script = "Hi {$name}, let's get {$company} taking orders online and directly on WhatsApp with instant checkout—so your catalog keeps selling even when physical shutters are down. Takes 3 weeks to go live. Want to see a live demo? ({$estimatorUrl})";
                } elseif ($segment === 'clinic') {
                    $script = "Hi Dr. {$name}, we built GutTalks telehealth clinic a frictionless booking and automated WhatsApp reminder system that cut no-shows and tripled repeat consultations. Can I send you a 2-minute video walkthrough? ({$websiteUrl}/portfolio/guttalks)";
                } elseif ($segment === 'coaching') {
                    $script = "Hi {$name}, replace paper fee registers and parent enquiry diaries with a single branded mobile app for attendance, automated fee reminders on WhatsApp, and student progress reports. ({$websiteUrl}/services/mobile-apps)";
                } else {
                    $script = "Hi {$name}, I noticed you're building {$company}. As a {$founderTitle}, I personally architect and ship modular web applications (Laravel 13, Vue 3, PostgreSQL) with sub-100ms response times and 100% test coverage. Every project includes weekly live staging demos and a 30-day post-launch warranty. ({$websiteUrl}/pricing)";
                }
            } elseif ($touch === 2) {
                $subject = "{$companyName} 2026 Price Book & Scope Estimator";
                $script = "Hi {$name}, {$founderName} here from {$companyName}. Just sharing our 2026 Price Book & Scope Estimator in case you want to ballpark numbers for {$company}: {$estimatorUrl}.\n\nHave 10 mins this week to review your bottleneck?";
            } elseif ($touch === 3) {
                $subject = "Architecture guide on eliminating errors";
                $script = "Hi {$name}, thought of your workflow — we just published an architecture guide on how teams are eliminating dispatch & booking errors: {$websiteUrl}/blog.\n\nWorth a quick 3-minute read.";
            } elseif ($touch === 4) {
                $subject = "Checking in on quarterly priority";
                $script = "Hi {$name}, {$founderName} here. Dropping a quick note to see if solving software bottlenecks for {$company} is still a priority for this quarter, or if you'd like to revisit next month?";
            } else {
        ...
        if ($apiKey && $request->boolean('use_ai', false)) {
            try {
                $res = Http::withHeaders([
                    ...
                    'messages' => [
                        ['role' => 'system', 'content' => "You are an elite B2B sales copywriter for {$companyName} software engineering studio. Refine this message to be ultra-crisp, authoritative, empathetic, and strictly under 110 words."],
