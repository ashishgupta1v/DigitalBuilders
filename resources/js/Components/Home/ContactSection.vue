    ...
    <div>
        ...
        <section id="contact" class="mt-20 grid gap-6 sm:mt-24 sm:gap-8 lg:grid-cols-[1.15fr_0.85fr]" data-reveal>
            <div class="db-antigravity-card rounded-3xl border border-slate-200 dark:border-[#b8c9e633] bg-white dark:bg-[#27374dde] p-6 sm:p-8 shadow-xl">
                ...
                <form id="home-contact-form" name="home_contact_form" action="/library/contact" method="post" class="mt-8 space-y-5" @submit.prevent="submitLead">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            @change="form.clearErrors('project_type')"
                            required
                    ...
                    <button
                        ...
                        type="submit"
                        class="db-action w-full rounded-full border border-transparent bg-[linear-gradient(95deg,#0284c7_0%,#4f46e5_48%,#7c3aed_100%)] dark:bg-[linear-gradient(95deg,#7ac4ff_0%,#9ba7ff_48%,#c593ff_100%)] px-6 py-3.5 text-sm font-bold text-white dark:text-[#1a2231] transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-70 shadow-lg cursor-pointer"
                    >
                    ...
                    <div v-if="form.recentlySuccessful" role="alert" aria-live="polite" class="rounded-2xl border border-emerald-500/40 bg-emerald-500/10 p-4 text-sm text-emerald-700 dark:text-emerald-200 space-y-3">
                        ...
                        <a :href="'https://wa.me/919087021592?text=' + encodeURIComponent('Hi Ashish, I just submitted a project inquiry on DigitalBuilders regarding ' + form.project_type + '. Let\'s connect!')" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 rounded-xl bg-[#25d366] px-4 py-2.5 text-xs font-bold text-white shadow hover:bg-[#20ba5a] transition">
