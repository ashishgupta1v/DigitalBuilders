...
const activeTab = ref<'cal' | 'whatsapp'>('cal');
const iframeLoading = ref(true);
const calUrl = ref('https://cal.com/ashishgupta/30min');

function handleIframeLoaded() {
    iframeLoading.value = false;
    trackEvent('cal_embed_loaded', { path: '/book' });
}
    ...
    <div class="min-h-screen bg-background text-foreground selection:bg-primary/20 selection:text-primary relative overflow-hidden flex flex-col justify-between">
        ...
        <header class="relative z-20 border-b border-border/80 bg-background/80 backdrop-blur-md">
            <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a
                        href="https://wa.me/919999999999?text=Hi%20Ashish,%20I'd%20like%20to%20schedule%20an%20architecture%20consultation"
                        target="_blank"
        ...
        <main id="main-content" class="relative z-10 mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 flex-1">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                ...
                <!-- Right Column: Interactive Booking Embed / Fast-Track Tabs -->
                <div class="lg:col-span-7">
                    <div class="relative overflow-hidden rounded-3xl border border-border bg-card text-card-foreground shadow-2xl">
                        ...
                        <div class="flex items-center justify-between border-b border-border p-4 sm:p-6 bg-secondary/20">
                            ...
                            <span class="hidden sm:inline-block text-[11px] font-mono text-muted-foreground">Auto-Timezone Sync</span>
                        </div>

                        <!-- Cal.com Slot Picker View -->
                        <div v-show="activeTab === 'cal'" class="p-4 sm:p-6 min-h-[580px] relative flex flex-col justify-center">
                            <div v-if="iframeLoading" class="absolute inset-0 flex flex-col items-center justify-center bg-card z-10 gap-3">
                                <div class="h-10 w-10 rounded-full border-4 border-primary/20 border-t-primary animate-spin" />
                                <p class="text-xs font-mono text-muted-foreground">Loading calendar availability...</p>
                            </div>

                            <iframe
                                :src="calUrl"
                                class="w-full h-[580px] border-0 rounded-2xl bg-card"
                                title="Select a 30-Minute Architecture Consultation Slot"
                            ...
                            <div class="mt-4 pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
                                <span>Calendar not loading in your browser?</span>
                                <a
                                    :href="calUrl"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    class="text-primary font-bold hover:underline inline-flex items-center gap-1 min-h-[44px] py-2"
                                >
                        ...
                        <div v-show="activeTab === 'whatsapp'" class="p-6 sm:p-10 space-y-6">
                            ...
                            <div class="text-center pt-2">
                                <a
                                    href="https://wa.me/919999999999?text=Hi%20Ashish,%20I'd%20like%20to%20discuss%20an%20architecture%20project"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    @click="handleWhatsAppDirect"
